...
func (a *LocalAuthorizer) Authorize(ctx context.Context, request Request) (Decision, error) {
	...
	for _, binding := range bindings {
		if !matches(binding.ResourceType, request.Resource.Type) ||
			!matches(binding.Resource, request.Resource.ID) {
			continue
		}
		for _, action := range strings.Split(binding.Actions, ",") {
			if matches(strings.TrimSpace(action), request.Action) {
