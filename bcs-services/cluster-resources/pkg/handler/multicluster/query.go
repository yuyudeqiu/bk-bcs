...
type Query interface {
	...
	FetchPreferred(ctx context.Context, gvr *schema.GroupVersionResource) (map[string]interface{}, error)
}
...
// FetchPreferred fetches multicluster resources by GVR.
...
// listResourcePreferred 列出多集群 GVR 资源
...
// listNamespaceResourcesPreferred 列出某个集群下某些命名空间的 GVR 资源
...
func checkMultiClusterAccess(ctx context.Context, kind string, clusters []*clusterRes.ClusterNamespaces) (
	...
	for _, v := range clusters {
		...
		// GVR 查询没有单一 kind，跳过共享集群的 kind 白名单检查。
		if kind == "" {
			newClusters = append(newClusters, v)
			continue
		}
