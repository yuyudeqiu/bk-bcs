...
func SearchBusiness(ctx context.Context, username, bizID string) (*SearchBusinessData, error) {
	// 获取超时时间
	...
	// 获取返回数据
	body, err := component.Request(req, timeout, config.GlobalConf.CMDB.Proxy,
...
func searchBusinessByIds(ctx context.Context, condition string, rules []map[string]interface{}) ([]BusinessData, error) {
	timeout := defaultTimeout
	if config.GlobalConf.CMDB.Timeout != 0 {
		timeout = config.GlobalConf.CMDB.Timeout
	}
