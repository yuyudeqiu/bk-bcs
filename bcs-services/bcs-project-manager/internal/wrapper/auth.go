...
func CheckUserPerm(ctx context.Context, req server.Request, userinfo utils.UserInfo) (bool, error) {
	logging.Info("CheckUserPerm: method/%s, username: %s, tenantId: %s",
		req.Method(), userinfo.BkUserName, userinfo.TenantId)

	if len(userinfo.BkUserName) == 0 {
	...
	if err != nil {
		return false, errorx.NewReadableErr(errorx.UnauthErr, err.Error())
	}

	allow, url, resources, err := callIAM(userinfo.BkUserName, user.GetTanantId(), action, *resourceID)
	if err != nil {
...
func callIAM(username, tenantId, action string, resourceID resourceID) (bool, string, []authutils.ResourceAction, error) {
	...
	user := utils.UserInfo{BkUserName: username, TenantId: tenantId}
