...
func (h BKMonitorHandler) GetClusterOverview(c *rest.Context) (ClusterOverviewMetric, error) {
	...
	promqlMap := map[string]string{
		"cpu_used":       `sum(rate(bkmonitor:container_cpu_usage_seconds_total{pod!="", bcs_cluster_id="%<clusterID>s", node!=""%<node>s}[2m]))`,
		...
		"memory_used":    `sum(avg_over_time(bkmonitor:container_memory_usage_bytes{pod!="", bcs_cluster_id="%<clusterID>s", node!=""%<node>s}[2m]))`,
...
func (h BKMonitorHandler) ClusterCPUUsage(c *rest.Context) (promclient.ResultData, error) {
	promql := `sum(rate(bkmonitor:container_cpu_usage_seconds_total{pod!="", bcs_cluster_id="%<clusterID>s", node!=""%<node>s}[2m])) / ` +
...
func (h BKMonitorHandler) ClusterMemoryUsage(c *rest.Context) (promclient.ResultData, error) {
	promql := `sum(avg_over_time(bkmonitor:container_memory_usage_bytes{pod!="", bcs_cluster_id="%<clusterID>s", node!=""%<node>s}[2m])) / ` +
