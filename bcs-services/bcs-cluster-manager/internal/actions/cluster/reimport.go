...
func (ra *ReimportClusterValidateAction) validate() error {
	var err error
	...
	// get cluster
	ra.cluster, err = ra.model.GetCluster(ra.ctx, ra.req.ClusterID)
	if err != nil {
		return err
	}
...
func (ra *ReimportClusterValidateAction) getClusterNodes() error {
	...
	// get cluster nodes
	ra.nodes = mergeClusterNodes(ra.cluster, cmNodes, k8sNodes)
